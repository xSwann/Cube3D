...
/*
 *  * get_color : renvoie la couleur du pixel situé à la position (x, y) dans la colonne
 * en cours de rendu. La fonction sélectionne la bonne texture selon la face du mur
 * touchée, calcule la coordonnée horizontale dans cette texture (tex_x), puis la
 * coordonnée verticale (tex_y) en fonction de la hauteur du mur affiché.

	// Déterminer la position de l’impact du rayon sur le mur.
	// Si le mur touché est vertical (side == 0), l’impact dépend de la coordonnée y du joueur.
	// Sinon, il dépend de la coordonnée x. On multiplie par la distance perpendiculaire pour
	...
	// il faut inverser l’axe x pour éviter que la texture ne soit affichée à l’envers.

	// La coordonnée y dans la texture est obtenue en prenant la partie entière de tex_pos.
	// L’opérateur & (bitwise AND) avec (tex->height - 1) d’alternative rapide à un modulo.
