 ...
 * - camera_x : de -1 (gauche de l’écran) à +1 (droite) pour interpoler la direction.
 ...
 * - delta_dist_x/delta_dist_y : distances à parcourir entre intersections (voir get_delta).
...
/*
 * init_dda : prépare les paramètres pour l’algorithme DDA (Digital Differential Analyzer).
 * On détermine dans quel sens (±1) le rayon traverse la grille pour les axes x et y.
 ...
 * +1 si le rayon se déplace vers le bas (dir_y >= 0) et -1 s’il se déplace vers le haut.
 *
 ...
 *     // on prend la différence entre la position en X du joueur et la coordonnée entière map_x,
 *     // puis on la multiplie par delta_dist_x pour la convertir en distance le long du rayon.
 ...
 *     // (map_x + 1.0 - pos_x) représente l’écart horizontal jusqu’au bord de la cellule,
...
// line_height Calcule la hauteur du mur en fonction de la distance (plus loin = plus petit)
// draw_start Positionne le haut du mur pour que l’image soit centrée verticalement
// if (draw_start < 0) // Si le début est au-dessus de l’écran, on l’ajuste  draw_start = 0;
 ...
 y = 0; // Commence au pixel supérieur de la colonne
    while (y < WIN_HEIGHT) // Parcours de chaque pixel verticalement
    {
        if (y < draw_start) // Si on est au-dessus du mur
            ...
            color = get_color(app, ray, y, line_height); // Sinon on calcule la couleur texturée du mur
        my_mlx_pixel_put(&app->frame, x, y, color); // On dessine le pixel dans l’image finale
